	...
	while (1)
	{
		switch (test)
		{
			...
			case 'R':
			{
				...
				for(;;)
				{
					uint16_t color = device->GetAnalogInput(Hal::DeviceInput::AnalogInputIndex::Color) / 11;
					uint8_t brightness = device->GetAnalogInput(Hal::DeviceInput::AnalogInputIndex::Brightness) / 16;
